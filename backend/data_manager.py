...
def get_connection():
    if RENDER_DB_URL:
        # 1. Modo PRODUCCIÓN (PostgreSQL)
        db_url = RENDER_DB_URL.replace("postgres://", "postgresql+psycopg2://", 1)
        try:
            # Retornamos la conexión bruta de Psycopg2
            engine = create_engine(db_url)
            return engine.raw_connection()
        except Exception as e:
            # Esto detendrá la aplicación si la conexión falla en Render
            raise ConnectionError(f"Error conectando a Postgres: {e}")
    else:
        # 2. Modo DESARROLLO LOCAL (FallBack a SQLite)
        # Asume que DB_PATH está definido o usa la ruta estándar
