...
def get_stocks_data(force_refresh=False):
    ...
    # 1. ACTUALIZAR CACHÉ (Solo lo necesario)
    ...
    if tickers_to_fetch and finnhub_client:
        ...
        for t in tickers_to_fetch:
            try:
                # Usamos el ticker original para consultar la API
                q = finnhub_client.quote(t)
                if q['c'] == 0 and q['pc'] == 0: continue
                ...
                cursor = conn.cursor()
                cursor.execute("""
                INSERT OR REPLACE INTO market_cache (ticker, price, day_change, day_change_pct, day_high, day_low, 
                market_cap, sector, country, summary, news, sentiment, last_updated) 
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?, datetime('now','localtime'))
                """, (
                    t, q['c'], q['d'], q['dp'], q['h'], q['l'], 
                    p.get('marketCapitalization',0), p.get('finnhubIndustry','N/A'), 
                    p.get('country','N/A'), p.get('currency','USD'), 
    ...
    # 2. LEER Y PROCESAR (Aquí aplicamos limpieza y corrección de tipo)
    query = """
    SELECT i.*, c.price as current_price, c.day_change, c.day_change_pct, c.sector, 
    ...
    for _, row in df.iterrows():
        ...
        # --- APLICAR CORRECCIONES VISUALES ---
        ticker_clean = clean_ticker_display(row['ticker'])
        final_type = detect_asset_type(row['ticker'], row['sector'])
        
        # Si la DB tiene el tipo incorrecto ("Stock"), lo corregimos visualmente
        asset_type_db = row['asset_type']
        if asset_type_db == 'Stock' and final_type != 'Stock':
            asset_type_db = final_type
            
        # Nombre display: Si es Crypto, usar el ticker limpio (BTC). Si es stock, usar ticker limpio también.
        ...
        results.append({
            'id': row['id'],
            'ticker': row['ticker'],        # Ticker real (para backend)
            'display_ticker': ticker_clean, # Ticker corto (para UI)
            'name': display_name,
            ...
            # Relleno de datos extra
            'day_high': row['day_high'] or 0, 'day_low': row['day_low'] or 0,
            'fiftyTwo_high':0, 'fiftyTwo_low':0, 'pe_ratio':0, 'dividend_yield':0, 'beta':0, 
            'country':'N/A', 'summary':'', 'news': news_data, 'sentiment': {}
        })
...
def get_investment_detail(asset_id):
    ...
    for asset in all_data:
        if asset['id'] == asset_id:
            # Aseguramos que el nombre y ticker estén limpios en el detalle también
