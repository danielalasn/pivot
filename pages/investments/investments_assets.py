...
def save_asset(n_clicks, ticker, shares, total_investment, signal):
    ...
    try:
        ...
        if shares_f <= 0:
            return no_update, html.Span("Las unidades deben ser mayores a cero.", className="text-danger")
        if total_investment_f <= 0:
            return no_update, html.Span("La inversión debe ser mayor a cero.", className="text-danger")
    ...
    if success:
        ...
        return no_update, html.Span(f"Error al guardar: {msg}", className="text-danger")
...
@callback(
    [Output("asset-update-signal", "data", allow_duplicate=True),
     Output("edit-asset-modal-msg", "children", allow_duplicate=True)],
    Input("btn-edit-save", "n_clicks"),
...
def save_edited_asset(n_clicks, shares, total_investment, asset_id, signal):
    if asset_id is None:
        return no_update, html.Span("Error: No se encontró el activo a editar.", className="text-danger")
    ...
    try:
        ...
        if shares_f <= 0 or total_investment_f <= 0:
            return no_update, html.Span("Las unidades y la inversión deben ser positivas.", className="text-danger")
    ...
    if success:
        return (signal + 1), html.Span("Posición actualizada.", className="text-success")
    else:
        return no_update, html.Span(f"Error al guardar: {msg}", className="text-danger")
